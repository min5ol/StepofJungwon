...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDetails> handleUsernameNotProvided(UsernameNotProvidedException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.BAD_REQUEST.value(), ex.getMessage(), request.getDescription(false));
    ...
    public ResponseEntity<ErrorDetails> handleInvalidCredentials(InvalidCredentialsException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.UNAUTHORIZED.value(), ex.getMessage(), request.getDescription(false));
    ...
    public ResponseEntity<ErrorDetails> handleLoginServerError(LoginServerException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.INTERNAL_SERVER_ERROR.value(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    // 회원가입 예외 처리
    @ExceptionHandler(SignupUsernameNotProvidedException.class)
    public ResponseEntity<ErrorDetails> handleSignupUsernameNotProvided(SignupUsernameNotProvidedException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.BAD_REQUEST.value(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }
    
    @ExceptionHandler(InvalidSignupUsernameException.class)
    public ResponseEntity<ErrorDetails> handleInvalidSignupUsername(InvalidSignupUsernameException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.BAD_REQUEST.value(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ErrorDetails> handleDuplicateUsername(DuplicateUsernameException ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.CONFLICT.value(), ex.getMessage(), request.getDescription(false));
    ...
    public ResponseEntity<ErrorDetails> handleGlobalException(Exception ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(HttpStatus.INTERNAL_SERVER_ERROR.value(), "서버 오류", request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
